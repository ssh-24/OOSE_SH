...
public class MemberDBManager
{
    ...
    boolean registerMemberInfo(Model.Member member)     //dB에 회원 정보를 저장
    {
        try
        {
            ...
            dbManager.setPstmt(dbManager.getConn().prepareStatement(query));     //DBConnect의 패키지명이 잘못돼서 나는 오류임 ㅇㅇ
            ...
            dbManager.setPstmt(dbManager.getConn().prepareStatement(query));
            dbManager.getPstmt().setString(1, member.getId());
